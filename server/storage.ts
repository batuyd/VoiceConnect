...
export class DatabaseStorage implements IStorage {
  ...
  async getServerInvitesByUser(userId: number): Promise<ServerInvite[]> {
    try {
      ...
      console.error('Error getting user server invites:', error);
      throw error;
    }
  }
  ...
  async getCoinProducts(): Promise<CoinProduct[]> {
    const products = await db.select().from(coinProducts);
    return products.map(product => ({
      ...product,
      icon: product.icon || 'coin' // Ensure icon is always present
    }));
  }
